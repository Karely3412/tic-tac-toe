...
def draw_moves_list():
   ...
   for player_idx in range(2):
      ...
      for move in moves_list[player_idx]:
         addstr(row, col + (player_idx * 4), str(move))
         row += 1
...
def draw_board_display():
   ...
   for i_row in range(3):
      for i_col in range(3):
         ...
         if letter == '':
            set_font(fg.green, text.dim)
            addstr(row + (2 * i_row), col + (i_col * 4), str(i_col + (i_row * 3)))
            reset()
         else:
            addstr(row + (2 * i_row), col + (i_col * 4), letter)
...
def print_many_games_scores(players, wins, wins2):
   ...
   addstr(row + 2, col + 2, player_names[0])
   addstr(row + 2, col + 27, player_names[1])

   col += 5
   row += 6
   addstr(row, col, f'{wins[0]:>4}')
   addstr(row+1, col, f'{wins[1]:>4}')
   addstr(row+2, col, f'{wins[2]:>4}')
   col += 10
   addstr(row, col, f'{wins2[1]:>4}')
   addstr(row+1, col, f'{wins2[0]:>4}')
   addstr(row+2, col, f'{wins2[2]:>4}')
   
   col += 15
   addstr(row, col, f'{wins2[0]:>4}')
   addstr(row+1, col, f'{wins2[1]:>4}')
   addstr(row+2, col, f'{wins2[2]:>4}')
   col += 10
   addstr(row, col, f'{wins[1]:>4}')
   addstr(row+1, col, f'{wins[0]:>4}')
   addstr(row+2, col, f'{wins[2]:>4}')

   col = board_offset_x + 10
   row += 4
   addstr(row, col, f'{(wins[0] + wins2[1]):>4}')
   addstr(row + 1, col, f'{(wins[1] + wins2[0]):>4}')
   addstr(row + 2, col, f'{(wins[2] + wins2[2]):>4}')
   
   col += 25
   addstr(row, col, f'{(wins[1] + wins2[0]):>4}')
   addstr(row + 1, col, f'{(wins[0] + wins2[1]):>4}')
   addstr(row + 2, col, f'{(wins[2] + wins2[2]):>4}')
...
def print_status_bar(val, total_val):
   ...
   addstr(row, col, val_box_str + remainder_str)
